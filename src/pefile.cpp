...
namespace pefile
{
  ...
  void PE::parse()
  {
    ...
    {
      auto oh_data = std::span<const uint8_t>(m_data.data() + optional_header_offset,
    ...
    {
      add_warning("Invalid type in Optional Header: 0x" + std::to_string(m_optional_header_32.Magic));
    }
    ...
    uint32_t num_dirs = is_pe32_plus() ? m_optional_header_64.NumberOfRvaAndSizes
                                       : m_optional_header_32.NumberOfRvaAndSizes;

    if (num_dirs > 0x10)
    {
      add_warning("Suspicious NumberOfRvaAndSizes: 0x" + std::to_string(num_dirs));
    }

    uint32_t dir_count = std::min(num_dirs, static_cast<uint32_t>(IMAGE_NUMBEROF_DIRECTORY_ENTRIES));
    for (uint32_t i = 0; i < dir_count; i++)
    {
      if (dir_offset + 8 > m_data_size)
        break;
  ...
  void PE::parse_sections(size_t offset, size_t max_offset)
  {
    ...
    for (uint16_t i = 0; i < m_file_header.NumberOfSections; i++)
    {
      ...
      if (section_offset + 40 > m_data_size)
        break;

      auto section_data = std::span<const uint8_t>(m_data.data() + section_offset,
      ...
      auto section_alignment = is_pe32_plus() ? m_optional_header_64.SectionAlignment
                                              : m_optional_header_32.SectionAlignment;
      auto file_alignment = is_pe32_plus() ? m_optional_header_64.FileAlignment
                                           : m_optional_header_32.FileAlignment;
      ...
      {
        add_warning("Suspicious value found parsing section " + std::to_string(i)
                    + ". VirtualSize is extremely large.");
      }

      if (adjust_section_alignment(section.VirtualAddress, section_alignment, file_alignment) > max_offset)
      {
        add_warning("Suspicious value found parsing section " + std::to_string(i)
                    + ". VirtualAddress is beyond limit.");
      }

      m_sections.push_back(section);
    }

    std::sort(m_sections.begin(), m_sections.end(), [](const SectionHeader& a, const SectionHeader& b) {
  ...
  std::span<const uint8_t> PE::get_data(uint32_t rva, std::optional<uint32_t> length) const
  {
    ...
    if (length)
    {
      ...
      if (file_offset >= m_data_size)
        throw PEFormatError("Offset out of bounds");
      return std::span<const uint8_t>(m_data.data() + file_offset, end - file_offset);
    }

    auto max_len = std::min(static_cast<uint32_t>(m_data_size), static_cast<uint32_t>(section_end))
                   - file_offset;
    if (file_offset >= m_data_size || max_len > m_data_size)
      throw PEFormatError("Offset out of bounds");
  ...
  {
    uint64_t current_imagebase = is_pe32_plus() ? m_optional_header_64.ImageBase
                                                : m_optional_header_32.ImageBase;
    int64_t relocation_difference = static_cast<int64_t>(new_image_base)
          ...
          while (entry_idx < reloc.entries.size())
          {
            ...
            else if (entry.type == IMAGE_REL_BASED_HIGH)
            {
              ...
              v = static_cast<uint16_t>(
                  (static_cast<uint32_t>(v) + (static_cast<uint32_t>(relocation_difference) >> 16)) & 0xFFFF);
            ...
            else if (entry.type == IMAGE_REL_BASED_LOW)
            {
              uint16_t v = get_word_at_rva(entry.rva);
              v = static_cast<uint16_t>(
                  (static_cast<uint32_t>(v) + static_cast<uint32_t>(relocation_difference & 0xFFFF)) & 0xFFFF);
              set_word_at_rva(entry.rva, v);
            }
            else if (entry.type == IMAGE_REL_BASED_HIGHLOW)
            {
              uint32_t v = get_dword_at_rva(entry.rva);
              v = static_cast<uint32_t>(static_cast<int64_t>(v) + relocation_difference);
              set_dword_at_rva(entry.rva, v);
            }
            else if (entry.type == IMAGE_REL_BASED_HIGHADJ)
            {
              if (entry_idx >= reloc.entries.size())
                break;
              const auto& next = reloc.entries[entry_idx];
              entry_idx++;
              uint16_t v = get_word_at_rva(entry.rva);
              uint32_t result = ((static_cast<uint32_t>(v) << 16) + next.rva
                                 + static_cast<uint32_t>(relocation_difference))
                                & 0xFFFF0000;
    ...
    for (auto& import_desc : m_imports)
    {
      ...
      {
        imp.address = static_cast<uint64_t>(static_cast<int64_t>(imp.address) + relocation_difference);
      }
    }
    ...
    if (m_tls_data)
    {
      ...
      m_tls_data->address_of_index = static_cast<uint64_t>(static_cast<int64_t>(m_tls_data->address_of_index)
  ...
  {
    auto size_of_image = is_pe32_plus() ? m_optional_header_64.SizeOfImage : m_optional_header_32.SizeOfImage;
    auto image_size = std::min(size_of_image, max_virtual_address);
    std::vector<uint8_t> mapped(image_size, 0);

    auto headers_size = is_pe32_plus() ? m_optional_header_64.SizeOfHeaders
                                       : m_optional_header_32.SizeOfHeaders;
    ...
    for (auto& section : m_sections)
    {
      if (section.VirtualAddress >= max_virtual_address)
        continue;
      if (section.VirtualAddress + section.SizeOfRawData > max_virtual_address)
        continue;
      if (static_cast<uint64_t>(section.PointerToRawData) + section.SizeOfRawData > m_data_size)
        continue;
      if (static_cast<uint64_t>(section.VirtualAddress) + section.SizeOfRawData > image_size)
        continue;
    ...
    if (image_base)
    {
      uint64_t current_imagebase = is_pe32_plus() ? m_optional_header_64.ImageBase
                                                  : m_optional_header_32.ImageBase;
      int64_t delta = static_cast<int64_t>(*image_base) - static_cast<int64_t>(current_imagebase);

      if (m_relocations.empty() && m_data_directories.size() > static_cast<size_t>(DirectoryEntry::BASERELOC)
          && m_data_directories[static_cast<size_t>(DirectoryEntry::BASERELOC)].Size)
      {
      ...
      constexpr uint32_t R_HIGH = 1, R_LOW = 2, R_HIGHLOW = 3, R_HIGHADJ = 4, R_DIR64 = 10;
      ...
      for (auto& reloc : m_relocations)
      {
        ...
        while (entry_idx < reloc.entries.size())
        {
          ...
          {
            if (rva + 2 > mapped.size())
              continue;
            uint16_t v;
            std::memcpy(&v, mapped.data() + rva, 2);
            v = static_cast<uint16_t>((static_cast<uint32_t>(v) + (static_cast<uint32_t>(delta) >> 16))
                                      & 0xFFFF);
          ...
          else if (entry.type == R_LOW)
          {
            ...
            uint16_t v;
            std::memcpy(&v, mapped.data() + rva, 2);
            v = static_cast<uint16_t>((static_cast<uint32_t>(v) + static_cast<uint32_t>(delta & 0xFFFF))
                                      & 0xFFFF);
            std::memcpy(mapped.data() + rva, &v, 2);
          }
          else if (entry.type == R_HIGHLOW)
          {
            if (rva + 4 > mapped.size())
              continue;
            uint32_t v;
          ...
          else if (entry.type == R_HIGHADJ)
          {
            ...
            uint16_t v;
            std::memcpy(&v, mapped.data() + rva, 2);
            uint32_t highadj = (((static_cast<uint32_t>(v) << 16) + next.rva + static_cast<uint32_t>(delta))
                                & 0xFFFF0000)
                               >> 16;
  ...
  std::span<const uint8_t> PE::get_overlay() const
  {
    ...
    for (auto& s : m_sections)
    {
      ...
      if (end > max_end && end <= UINT32_MAX)
        max_end = static_cast<uint32_t>(end);
    }
    if (max_end >= m_data_size)
      return {};
  ...
  bool PE::is_driver() const
  {
    ...
    if (sub == 1)
      return true;
    if (sub == 10 || sub == 11 || sub == 12 || sub == 13)
      return true;
    return false;
  }
  ...
  bool PE::verify_checksum() const
  {
    return generate_checksum()
           == (is_pe32_plus() ? m_optional_header_64.CheckSum : m_optional_header_32.CheckSum);
  }
  ...
  {
    auto offset = get_offset_from_rva(rva);
    if (offset + data.size() > m_data_size)
      return false;
  ...
  {
    return set_bytes_at_rva(rva, std::span<const uint8_t>(reinterpret_cast<const uint8_t*>(&word), 2));
  }

  bool PE::set_dword_at_rva(uint32_t rva, uint32_t dword)
  {
    return set_bytes_at_rva(rva, std::span<const uint8_t>(reinterpret_cast<const uint8_t*>(&dword), 4));
  }

  bool PE::set_qword_at_rva(uint32_t rva, uint64_t qword)
  {
    return set_bytes_at_rva(rva, std::span<const uint8_t>(reinterpret_cast<const uint8_t*>(&qword), 8));
  }

  bool PE::set_word_at_offset(uint32_t offset, uint16_t word)
  {
    return set_bytes_at_offset(offset, std::span<const uint8_t>(reinterpret_cast<const uint8_t*>(&word), 2));
  }

  bool PE::set_dword_at_offset(uint32_t offset, uint32_t dword)
  {
    return set_bytes_at_offset(offset, std::span<const uint8_t>(reinterpret_cast<const uint8_t*>(&dword), 4));
  }

  bool PE::set_qword_at_offset(uint32_t offset, uint64_t qword)
  {
    return set_bytes_at_offset(offset, std::span<const uint8_t>(reinterpret_cast<const uint8_t*>(&qword), 8));
  }
  ...
  bool PE::set_version_string(const std::string& key, const std::string& value)
  {
    ...
    if (it == m_version_info->entries.end())
      return false;
    ...
    if (!set_bytes_at_offset(entry.value_offset, std::span<const uint8_t>(padded.data(), padded.size())))
    {
      return false;
    }
  ...
  std::vector<ImportDescData> PE::parse_import_directory(uint32_t rva, uint32_t size)
  {
    ...
    while (true)
    {
      if (rva + desc_size > m_data_size)
        break;
      if (rva + desc_size > end_rva)
        break;
      ...
      try
      {
        import_data = parse_imports(import_desc.OriginalFirstThunk,
  ...
  std::vector<ImportData> PE::parse_imports(uint32_t original_first_thunk,
                                            ...
                                            uint32_t max_length)
  {
    ...
    while (ilt_rva != 0 || iat_rva != 0)
    {
      ...
      if (max_length > 0 && current_rva >= start_rva + max_length)
        break;

      if (current_rva + entry_size > m_data_size)
        break;
      ...
      uint64_t imp_address = current_rva
                             + (is_pe32_plus() ? m_optional_header_64.ImageBase
                                               : m_optional_header_32.ImageBase);
  ...
  std::optional<ExportDirData> PE::parse_export_directory(uint32_t rva, uint32_t size)
  {
    ...
    for (uint32_t i = 0;
         i < std::min(export_dir.NumberOfNames, length_until_eof(export_dir.AddressOfNames) / 4);
         i++)
    {
      if (addr_of_name_ordinals + i * 2 + 2 > m_data_size)
        break;
  ...
  std::vector<DebugData> PE::parse_debug_directory(uint32_t rva, uint32_t size)
  {
    ...
    for (uint32_t idx = 0; idx < count; idx++)
    {
      ...
      if (dbg_rva + dbg_size > m_data_size)
        break;
  ...
  std::vector<BaseRelocationData> PE::parse_relocations_directory(uint32_t rva, uint32_t size) const
  {
    ...
    while (rva < end)
    {
      if (rva + 8 > m_data_size)
        break;

      auto rlc = ImageBaseRelocation::parse(m_data, get_offset_from_rva(rva));
      if (rlc.VirtualAddress == 0 && rlc.SizeOfBlock == 0)
        break;
      if (rlc.SizeOfBlock < 8 || rlc.SizeOfBlock > m_data_size)
        break;
  ...
  std::optional<VersionInfo> PE::parse_version_information(uint32_t rva)
  {
    ...
    auto stringfileinfo_offset = dword_align(ffi_offset + (value_length ? value_length : 52), rva);
    ...
    while (start_offset + stringfileinfo_offset + 6 <= m_data_size)
    {
      ...
      if (sf_length == 0 || sf_offset + sf_length > m_data_size)
        break;
      ...
      {
        auto st_offset = dword_align(stringfileinfo_offset + 6
                                         + 2 * (static_cast<uint32_t>(sf_key.size()) + 1),
                                     rva);

        while (start_offset + st_offset + 6 <= m_data_size && st_offset < stringfileinfo_offset + sf_length)
        {
          auto st_abs = start_offset + st_offset;
          uint16_t st_length = get_word_at_offset(st_abs);
          if (st_length == 0 || st_abs + st_length > m_data_size)
            break;

          std::string st_key = get_string_u_at_rva(get_rva_from_offset(st_abs + 6));

          // Parse individual String entries within the StringTable
          auto str_offset = dword_align(st_offset + 6 + 2 * (static_cast<uint32_t>(st_key.size()) + 1), rva);

          while (start_offset + str_offset + 6 <= m_data_size && str_offset < st_offset + st_length)
          {
            ...
            if (str_length == 0 || str_abs + str_length > m_data_size)
              break;

            auto str_key_offset = str_abs + 6;
            std::string str_key = get_string_u_at_rva(get_rva_from_offset(str_key_offset));

            if (!str_key.empty() && str_value_length > 0)
            {
              auto val_rva = get_rva_from_offset(
                  dword_align(str_offset + 6 + 2 * (static_cast<uint32_t>(str_key.size()) + 1), rva));
  ...
  std::vector<DelayImportDescData> PE::parse_delay_import_directory(uint32_t rva, uint32_t size)
  {
    ...
    while (true)
    {
      ...
      if (import_desc.grAttrs() == 0 && m_file_header.Machine == static_cast<uint16_t>(MachineType::I386))
      {
      ...
      catch (const PEFormatError& e)
      {
        add_warning("Error parsing the Delay import directory. "
                    "Invalid import data at RVA: 0x"
                    + std::to_string(rva) + " (" + e.what() + ")");
      }
  ...
  std::optional<ResourceDirData> PE::parse_resources_directory(
      uint32_t rva, uint32_t size, uint32_t base_rva, int level, std::vector<uint32_t> dirs)
  {
    if (level > static_cast<int>(MAX_RESOURCE_DEPTH))
    {
      add_warning("Error parsing the resources directory. "
                  "Excessively nested table depth "
                  + std::to_string(level));
      return std::nullopt;
    }
    ...
    if (number_of_entries > MAX_ALLOWED_RESOURCE_ENTRIES)
    {
      add_warning("Error parsing the resources directory. "
                  ...
                  + std::to_string(number_of_entries) + " entries");
      return std::nullopt;
    }
    ...
    for (uint32_t idx = 0; idx < number_of_entries; idx++)
    {
      ...
      if (res.is_directory())
      {
        ...
        auto subdir = parse_resources_directory(subdir_rva, size - (rva - base_rva), base_rva, level + 1, dirs);

        if (subdir)
        {
          dir_entries.push_back(ResourceDirEntryData{entry_rva,
                                                     entry_name,
                                                     entry_id,
                                                     std::make_shared<ResourceDirData>(*subdir),
                                                     nullptr});
        }
        else
        {
      ...
      else
      {
        ...
        if (data_entry)
        {
          ...
          dir_entries.push_back(ResourceDirEntryData{entry_rva, entry_name, entry_id, nullptr, entry_data});
        }
        else
        {
          break;
  ...
  std::string PE::get_imphash() const
  {
    ...
    for (auto& entry : m_imports)
    {
      ...
      std::transform(libname.begin(), libname.end(), libname.begin(), [](unsigned char c) {
      ...
      for (auto& imp : entry.imports)
      {
        ...
        std::transform(funcname.begin(), funcname.end(), funcname.begin(), [](unsigned char c) {
          return std::tolower(c);
        });
  ...
  {
    if (!m_exports || m_exports->symbols.empty())
      return "";
    std::string result;
  ...
  std::string PE::dump_info() const
  {
    ...
    for (auto& s : m_sections)
    {
      ss << "  " << s.name() << ": VA=0x" << std::hex << s.VirtualAddress << " Size=0x" << s.SizeOfRawData
  ...
  std::optional<RichHeaderData> PE::parse_rich_header()
  {
    ...
    if (data.size() >= 4)
    {
      if (data[0] != (DANS ^ checksum) || data[1] != checksum || data[2] != checksum || data[3] != checksum)
      {
  ...
  {
    if (!m_rich_header || m_rich_header->clear_data.empty())
      return "";
