...
namespace pefile
{
  void SigDB::load(const std::string& filename)
  {
  ...
  void SigDB::load(std::span<const uint8_t> data)
  {
    std::string content(reinterpret_cast<const char*>(data.data()), data.size());
    load_internal(content);
  }

  void SigDB::load_internal(std::string_view data)
  {
    ...
    for (auto it = begin; it != end; ++it)
    {
      ...
      bool section_start_only = match.size() > 5 && (match[5].str() == "True" || match[5].str() == "true");

      // Remove spaces from signature
      signature.erase(std::remove(signature.begin(), signature.end(), ' '), signature.end());

      auto& target_tree = section_start_only
                              ? m_signtree_section_start
                              : (ep_only ? m_sigtree_ep_only_true : m_sigtree_ep_only_false);
      ...
      for (size_t i = 0; i + 1 < signature.size(); i += 2)
      {
        ...
        {
          uint8_t byte_val = 0;
          auto [ptr, ec] = std::from_chars(byte_str.data(), byte_str.data() + 2, byte_val, 16);
          if (ec == std::errc())
          {
  ...
  std::string SigDB::match(const PE& pe, bool ep_only, bool section_start_only) const
  {
  ...
  std::vector<std::pair<uint32_t, std::string> > SigDB::match_all(const PE& pe,
                                                                  bool ep_only,
                                                                  bool section_start_only) const
  {
    std::vector<std::pair<uint32_t, std::string> > results;

    auto overlay_offset = pe.get_overlay_data_start_offset();
    auto max_offset = overlay_offset.value_or(
        static_cast<uint32_t>(pe.dos_header().e_lfanew + 4 + pe.file_header().SizeOfOptionalHeader));

    const TrieTree* tree = &m_sigtree_ep_only_true;
    if (section_start_only)
      tree = &m_signtree_section_start;
    else if (!ep_only)
      tree = &m_sigtree_ep_only_false;

    if (!*tree)
      return results;

    auto ep_offset = pe.get_offset_from_rva(pe.is_pe32_plus() ? pe.optional_header_64().AddressOfEntryPoint
                                                              : pe.optional_header_32().AddressOfEntryPoint);

    if (ep_only)
    {
      auto data = pe.get_data(ep_offset);
      auto matches = match_sigtree(*tree, data, 0);
      for (auto& m : matches)
        if (!m.empty())
  ...
  std::string SigDB::match_data(std::span<const uint8_t> code_data,
                                            bool ep_only,
                                            bool /*section_start_only*/) const
  {
    const TrieTree* tree = ep_only ? &m_sigtree_ep_only_true : &m_sigtree_ep_only_false;
    if (!*tree)
      return "";

    auto matches = match_sigtree(*tree, code_data, 0);
    if (!matches.empty() && !matches[0].empty())
      return matches[0][0];
    return "";
  }

  std::vector<std::vector<std::string> > SigDB::match_sigtree(const TrieTree& tree,
                                                                                 ...
                                                                                 size_t depth) const
  {
    ...
    {
      auto sub = match_sigtree(it->second, data, depth + 1);
    ...
    {
      auto sub = match_sigtree(wc->second, data, depth + 1);
      results.insert(results.end(), sub.begin(), sub.end());
    }
  ...
  std::string SigDB::gen_ep_sig(const PE& pe,
                                                       const std::string& name,
                                                       size_t sig_length) const
  {
    auto ep = pe.get_offset_from_rva(pe.is_pe32_plus() ? pe.optional_header_64().AddressOfEntryPoint
                                                       : pe.optional_header_32().AddressOfEntryPoint);
  ...
  std::vector<std::string> SigDB::gen_section_sigs(const PE& pe,
                                                   const std::string& name,
                                                   size_t sig_length) const
  {
  ...
  bool is_probably_packed(const PE& pe, double entropy, double threshold)
  {
    ...
    if (total_size == 0)
      return false;
    return static_cast<double>(high_entropy_size) / total_size > threshold;
  }
