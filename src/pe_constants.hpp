...
namespace pefile
{
  ...
  constexpr uint32_t MAX_DLL_LENGTH = 0x200;
  constexpr uint32_t MAX_IMPORT_NAME_LENGTH = 0x200;
  constexpr uint32_t MAX_SYMBOL_NAME_LENGTH = 0x200;
  constexpr uint32_t MAX_SECTIONS = 0x800;
  constexpr uint32_t MAX_RESOURCE_ENTRIES = 0x8000;
  constexpr uint32_t MAX_RESOURCE_DEPTH = 32;
  constexpr uint32_t MAX_SYMBOL_EXPORT_COUNT = 0x2000;
  constexpr uint32_t MIN_VALID_FILE_ALIGNMENT = 0x200;
  constexpr uint32_t SECTOR_SIZE = 0x200;
  constexpr uint32_t VS_VERSION_INFO_MAGIC_LEN = 6;

  constexpr uint16_t IMAGE_DOS_SIGNATURE = 0x5A4D;
  constexpr uint16_t IMAGE_DOSZM_SIGNATURE = 0x4D5A;
  constexpr uint32_t IMAGE_NT_SIGNATURE = 0x00004550;
  constexpr uint16_t IMAGE_NE_SIGNATURE = 0x454E;
  constexpr uint16_t IMAGE_LE_SIGNATURE = 0x454C;
  constexpr uint16_t IMAGE_LX_SIGNATURE = 0x584C;
  constexpr uint16_t IMAGE_TE_SIGNATURE = 0x5A56;

  constexpr uint32_t IMAGE_NUMBEROF_DIRECTORY_ENTRIES = 16;
  constexpr uint64_t IMAGE_ORDINAL_FLAG = 0x80000000;
  constexpr uint64_t IMAGE_ORDINAL_FLAG64 = 0x8000000000000000;
  constexpr uint16_t OPTIONAL_HEADER_MAGIC_PE = 0x10B;
  constexpr uint16_t OPTIONAL_HEADER_MAGIC_PE_PLUS = 0x20B;
  ...
  enum class DirectoryEntry : uint32_t
  {
    EXPORT = 0,
    IMPORT = 1,
    RESOURCE = 2,
  ...
  enum class ImageCharacteristic : uint16_t
  {
  ...
  inline std::string_view image_characteristic_name(ImageCharacteristic c)
  {
    switch (c)
    {
      case ImageCharacteristic::RELOCS_STRIPPED:
        return "IMAGE_FILE_RELOCS_STRIPPED";
      case ImageCharacteristic::EXECUTABLE_IMAGE:
        return "IMAGE_FILE_EXECUTABLE_IMAGE";
      case ImageCharacteristic::LINE_NUMS_STRIPPED:
        return "IMAGE_FILE_LINE_NUMS_STRIPPED";
      case ImageCharacteristic::LOCAL_SYMS_STRIPPED:
        return "IMAGE_FILE_LOCAL_SYMS_STRIPPED";
      case ImageCharacteristic::AGGRESIVE_WS_TRIM:
        return "IMAGE_FILE_AGGRESIVE_WS_TRIM";
      case ImageCharacteristic::LARGE_ADDRESS_AWARE:
        return "IMAGE_FILE_LARGE_ADDRESS_AWARE";
      case ImageCharacteristic::_16BIT_MACHINE:
        return "IMAGE_FILE_16BIT_MACHINE";
      case ImageCharacteristic::BYTES_REVERSED_LO:
        return "IMAGE_FILE_BYTES_REVERSED_LO";
      case ImageCharacteristic::_32BIT_MACHINE:
        return "IMAGE_FILE_32BIT_MACHINE";
      case ImageCharacteristic::DEBUG_STRIPPED:
        return "IMAGE_FILE_DEBUG_STRIPPED";
      case ImageCharacteristic::REMOVABLE_RUN_FROM_SWAP:
        return "IMAGE_FILE_REMOVABLE_RUN_FROM_SWAP";
      case ImageCharacteristic::NET_RUN_FROM_SWAP:
        return "IMAGE_FILE_NET_RUN_FROM_SWAP";
      case ImageCharacteristic::SYSTEM:
        return "IMAGE_FILE_SYSTEM";
      case ImageCharacteristic::DLL:
        return "IMAGE_FILE_DLL";
      case ImageCharacteristic::UP_SYSTEM_ONLY:
        return "IMAGE_FILE_UP_SYSTEM_ONLY";
      case ImageCharacteristic::BYTES_REVERSED_HI:
  ...
  inline std::vector<std::pair<std::string, uint16_t> > retrieve_image_characteristics()
  {
    return {
  ...
  enum class SectionCharacteristic : uint32_t
  {
    CNT_CODE = 0x00000020,
  ...
  enum class DllCharacteristic : uint16_t
  {
  ...
  enum class ExDllCharacteristic : uint16_t
  {
  ...
  inline uint32_t adjust_section_alignment(uint32_t val,
  ...
  inline bool power_of_two(uint32_t val)
    { return val != 0 && (val & (val - 1)) == 0; }

  inline uint32_t count_zeroes(std::span<const uint8_t> data)
    { return static_cast<uint32_t>(std::count(data.begin(), data.end(), 0)); }
  ...
  inline uint32_t dword_align(uint32_t offset, uint32_t base)
  {
    return ((offset + base + 3) & 0xFFFFFFFC) - (base & 0xFFFFFFFC);
  }

  inline std::string human_readable_size(uint64_t value)
  {
